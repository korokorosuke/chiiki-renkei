...
export class WebAppRepository implements IWebAppRepository {
  ...
  async insert(val: WebAppointment): Promise<boolean> {
    ...
    if(val.date === DATE_EMPTY || await repo.countUp(val.department.id, val.date, val.dr.id, val.time, false)){
      ...
      const res: number = await db.transaction(async (tx) => {
        const res1 = (await tx.insert(webAppointment).values(this.toData(val))).rowsAffected;
        if(res1 >= 1){
      ...
      if(res >= 1){
        ...
        await repo.countUp(val.department.id, val.date, val.dr.id, val.time, false);
        return false;
      }
    }
    return false;
  ...
  async update(val: WebAppointment): Promise<boolean> {
    ...
    const db = await this.database.open();
    const res: number = await db.transaction(async (tx) => {
      const res1 = (await tx.update(webAppointment).set(this.toDataWithoutKey(val))
        .where(eq(webAppointment.id, val.id))).rowsAffected;
      if(res1 >= 1){
