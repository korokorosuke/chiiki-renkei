...
export class StaffRepository implements IStaffRepository {
  ...
  toDataWithoutKey(act: Staff): Partial<StaffData> {
  ...
  async insert(val: Staff): Promise<boolean> {
    ...
    const res = (await db.insert(staff).values(this.toData(val))).rowsAffected;
    return res >= 1;
  }

  async update(val: Staff): Promise<boolean> {
    const db = await this.database.open();
    const res = (await db.update(staff).set(this.toDataWithoutKey(val))
      .where(
        and(
