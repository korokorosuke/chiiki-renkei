...
export class QuestionnaireRepository implements IQuestionnaireRepository {
  ...
  async insert(val: Questionnaire): Promise<boolean> {
    ...
    const res: number = await db.transaction(async (tx) => {
      const res1 = (await tx.insert(questionnaire).values(this.toData(val))).rowsAffected;
      if(res1 >= 1){
        let res2 = (await tx.insert(questionnaireDept)
        ...
        if(val.items.length > 0){
          res2 = (await tx.insert(question)
            .values(val.items.map((item, i) => this.toQuestionData(item, val.id, i)))).rowsAffected;
          if(res2 === 0){
            tx.rollback();
            return 0;
          }
          ...
          if(conds.length > 0){
            res2 = (await tx.insert(questionCondition)
              .values(conds)).rowsAffected;
            if(res2 === 0){
              tx.rollback();
              return 0;
            }
          ...
          if(choices.length > 0){
            res2 = (await tx.insert(questionChoice)
              .values(choices)).rowsAffected;
            if(res2 === 0){
              tx.rollback();
              return 0;
            }
  ...
  async update(val: Questionnaire): Promise<boolean> {
    const db = await this.database.open();
    const res: number = await db.transaction(async (tx) => {
      const res1 = (await tx.update(questionnaire).set(this.toDataWithoutKey(val))
        .where(and(
      ...
      if(res1 >= 1){
        ...
        if(val.items.length > 0){
          let res2 = (await tx.insert(question)
            .values(val.items.map((item, i) => this.toQuestionData(item, val.id, i)))).rowsAffected;
          if(res2 === 0){
            tx.rollback();
            return 0;
          }
          res2 = (await tx.insert(questionnaireDept)
            .values(val.depts.map(dept=>({
              questionnaireId: val.id,
              deptId: dept,
            })))).rowsAffected;
