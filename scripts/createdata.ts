...
const patient: Patient[] = [
    {
        ...
        tel2: "",
        address: {postalCode: "710-0101", name: "岡山県岡山市",
            plus: "岡山１－３－１０"}, memo: ""
    },
    {
    ...
    },
    {
        ...
        tel2: "",
        address: {postalCode: "713-0101", name: "岡山県岡山市",
            plus: "北区１－３－１３"}, memo: ""
    },
    {
...
const facdept = ["内科", "消化器内科", "呼吸器内科", "糖尿病内科", "腎臓内科",
    "内分泌代謝科", "循環器内科", "脳神経内科",
...
async function main(){
    ...
    department.forEach(async (d)=>{
        await kv.set([BASE, "department", d.id], d);
    });
    console.log("department end");
    console.log("dr create ...");
    dr.forEach(async (d)=>{
        await kv.set([BASE, "dr", d.id], d);
        await kv.set([BASE, "dr_dept", d.department, d.id], d);
    });
    console.log("dr end");
    console.log("facility create ...");
    facility.forEach(async (d)=>{
        await kv.set([BASE, "facility", d.id], d);
    });
    console.log("facility end");
    console.log("patient create ...");
    patient.forEach(async (d)=>{
        await kv.set([BASE, "patient", d.id], d);
    });
    console.log("patient end");
    console.log("staff create ...");
    staff.forEach(async (d)=>{
        await kv.set([BASE, "staff", d.id], d);
        await kv.set([BASE, "staff_fac", d.facilityId, d.id], d);
    });
    console.log("staff end");
    console.log("user create ...");
    user.forEach(async (d)=>{
        await kv.set([BASE, "user", d.id], d);
    });
    console.log("user end");
    console.log("due create ...");
    due.forEach(async (d)=>{
        await kv.set([BASE, "due", d.id], d);
    })
    console.log("due end");
    console.log("purpose create ...");
    await kv.set([BASE, "purpose"], purpose);
    console.log("purpose end");
    console.log("kind create ...");
    await kv.set([BASE, "kind"], kind);
    console.log("kind end");
    console.log("post create ...");
    await kv.set([BASE, "post"], post);
    console.log("post end");
    console.log("means create ...");
    await kv.set([BASE, "means"], means);
    console.log("means end");
    console.log("class create ...");
    await kv.set([BASE, "class"], classes);
    console.log("class end");
    console.log("facdept create ...");
    await kv.set([BASE, "facdept"], facdept);
